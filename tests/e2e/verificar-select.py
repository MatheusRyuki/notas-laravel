...
try:
    ...
    for largura in (1440, 390, 320):
        cdp.call("Emulation.setDeviceMetricsOverride", {"width": largura, "height": 1000, "deviceScaleFactor": 1, "mobile": largura < 760})
        cdp.navigate("/")
        time.sleep(0.4)
        cdp.evaluate("document.querySelector('.nova-nota').click()")
        time.sleep(0.4)
        resultado = cdp.evaluate("""(async () => {
            const campo = document.querySelector('#criar-tipo');
            campo.focus();
            await new Promise(resolve => setTimeout(resolve, 250));
            const css = getComputedStyle(campo);
            const r = campo.getBoundingClientRect();
            return {ativo: document.activeElement.id, foco: campo.matches(":focus"), raio: css.borderRadius, borda: css.borderTopColor,
                seta: css.paddingRight, rotulo: campo.labels[0].textContent,
                cabe: r.left >= 0 && r.right <= innerWidth};
        })()""")
        assert resultado["raio"] == "8px" and resultado["borda"] == "rgb(96, 121, 100)", resultado
        assert float(resultado["seta"].replace("px", "")) >= 38 and resultado["cabe"], resultado
        assert resultado["rotulo"] == "Tipo", resultado
        cdp.call("Input.dispatchKeyEvent", {"type":"keyDown", "key":"ArrowDown", "code":"ArrowDown", "windowsVirtualKeyCode":40})
        cdp.call("Input.dispatchKeyEvent", {"type":"keyUp", "key":"ArrowDown", "code":"ArrowDown", "windowsVirtualKeyCode":40})
        time.sleep(0.2)
        assert cdp.evaluate("document.querySelector('#criar-tipo').value") == "lista"
        cdp.screenshot(f"padronizacao-select-{largura}.png")
        print(json.dumps({"largura": largura, "estilo": resultado, "teclado": True}))
finally:
    chrome.terminate()
